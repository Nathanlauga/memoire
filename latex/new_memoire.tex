...
\paragraph{}
Dans le contexte actuel en France, les taux de crédits immobilier sont extrèmement bas : allant de 1,25\% pour un bon taux sur 15 ans jusqu'à 1,65\% pour un bon taux sur 25 ans (Source meilleurtaux.com, Juin 2020). La baisse des taux entraîne logiquement une baisse des gains pour les banques Françaises. De plus, la crise des Subprimes de 2007 aux USA a montrée une défaillance sur les contrôles pour accorder ou non un crédit immobilier. Bien qu'en Europe les contrôles ne sont pas les mêmes, la crise a également impacté l'Union Européenne. Avec ces informations, nous comprenons bien le besoin de s'assurer qu'un client puisse être en capacité de rembourser son crédit.

\paragraph{}
Maintenant, si nous nous plaçons du point de vue des clients qui font la demande de crédit immobilier, un refus a une incidence sociale directe sur la vie du client. En effet, il peut s'agir de renoncer à un projet de vie ou encore à un investissement qui, sur le long terme, peut permettre d'avoir un patrimoine immobilier qui génère du capital.

\paragraph{}
Cette problématique possède pousse à savoir si le modèle n'est pas biaisé, qu'il n'avantage pas plus un homme qu'une femme. De plus, en cas de refus nous pouvons imaginer que le client soit surpris et demande une explication. Par conséquent, le modèle doit être capable de fournir une explication cohérente avec le résultat obtenu. 

\subsubsection{Jeu de données : Home Credit Default Risk}

\paragraph{}
Pour entrainer notre modèle, nous allons utiliser les données issues de la compétition Kaggle : ``Home Credit Default Risk''\footnote{https://www.kaggle.com/c/home-credit-default-risk/}. Les données sont fournies par Home Credit, un service dédié à l'octroi de lignes de crédit (prêts) à la population non bancarisée. Prévoir si un client va ou non rembourser un prêt ou s'il a des difficultés est un besoin essentiel des entreprises, c'est pour cela que Home Credit a organisé ce concours sur Kaggle pour voir quels types de modèles la communauté d'apprentissage machine peut développer pour l'aider dans cette tâche.


\paragraph{}
Le jeu de données est composé de sept sources différentes :


\begin{itemize}
  \item \textbf{application\_train} : Il s'agit du jeu de données principal avec des informations sur chaque demande de prêt chez Home Credit. Chaque prêt a sa propre ligne et est identifié par la variable SK\_ID\_CURR. Les données relatives aux demandes de formation sont accompagnées de la mention TARGET indiquant 0 : le prêt a été remboursé ou 1 : le prêt n'a pas été remboursé.
  \item  \textbf{bureau} : Ces données concernent les crédits antérieurs du client auprès d'autres institutions financières. Chaque crédit précédent a sa propre ligne dans bureau, mais un prêt dans les données de demande peut avoir plusieurs crédits précédents.
  \item \textbf{bureau\_balance} : Nous trouvons ici, les données mensuelles concernant les crédits précédents en bureau. Chaque ligne correspond à un mois d'un crédit antérieur, et un seul crédit antérieur peut avoir plusieurs lignes, une pour chaque mois de la durée du crédit.
  \item \textbf{previous\_application} : Les demandes précédentes de prêts au crédit immobilier des clients dont les données de demande contiennent des prêts. Chaque prêt actuel dans les données de demande peut avoir plusieurs prêts précédents. Chaque demande précédente comporte une ligne et est identifiée par la caractéristique SK\_ID\_PREV.
  \item \textbf{POS\_CASH\_BALANCE} : Il s'agit des données mensuelles sur les prêts antérieurs au point de vente ou en espèces que les clients ont contractés auprès de Home Credit. Chaque ligne correspond à un mois d'un précédent prêt sur le lieu de vente ou en espèces, et un seul prêt précédent peut comporter plusieurs lignes.
  \item \textbf{credit\_card\_balance} : Les données mensuelles sur les cartes de crédit que les clients ont utilisées précédemment dans le cadre du crédit immobilier. Chaque ligne correspond à un mois de solde de carte de crédit, et une seule carte de crédit peut comporter plusieurs lignes.
  \item \textbf{installments\_payment} : L'historique des paiements pour les prêts précédents au Home Credit. Il y a une ligne pour chaque paiement effectué et une ligne pour chaque paiement manqué. 
\end{itemize}

\paragraph{}
Afin de rendre l'expérience plus simple, nous allons principalement exploité le jeu de données principal soit \textbf{application\_train} dans lequel nous avons la variable pour la prédiction (\textit{TARGET}) et celle pour le genre du demandeur de crédit (\textit{CODE\_GENDER}).

\paragraph{}
Dans le détail le jeu de données que nous allons utilisé possède 122 colonnes sur lesquels nous allons pouvoir entraîner nos modèles.

\subsubsection{Algorithmes choisis}

\paragraph{}
La problématique consiste à prédire la probabilité d'un ou d'une client.e à rembourser un crédit immobilier ou non. Il s'agit donc d'une classification binaire : le résultat sera situé entre 0 et 1 et nous estimerons qu'une probabilité de non remboursement supérieure à 50\% signifie que le client ne pourra pas rembourser.
...
\paragraph{}
Voici la liste d'algorithme à tester en recherche aléatoire des hyper-paramètres :
...
\paragraph{}
Le choix de ces algorithmes vient de leur performance dans le domaine de la classification binaire, ainsi que de leur utilisation par les participants de la compétition sur le site de Kaggle. 
...
\paragraph{}
Pour le client, c'est la précision qui importe. Si la précision est faible alors les clients auront tendance à être considéré comme risqués et donc risque d'avoir des taux plus élevés ou tout simplement un refus de crédit.
...
\begin{itemize}
  \item $M$ : Un modèle prédisant si une personne peut rembouser un prêt immobilier
  ...
  \item $P(X)$ : Probabilité de rembourser un crédit
  \item $P(X | F)$ : Probabilité de rembourser un prêt sachant que la personne demandant est une femme
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Bob} :  En utilisant les variables $X$, utiliser le modèle $M$ tel que $P(X|F) = \max(0, P(X|F) - 0,2) \text{ ET } S(M) >= 0_{min}$.
...
\paragraph{}
Nous avons donc une morale de contrôle (Alice) qui a pour objectif d'uniquement réussir à compléter le critère de validation (qui est une situation classique pour un projet de Machine Learning). Ensuite, la morale discriminante (Bob), qui réduira de 20\% la probabilité sortante du modèle si la personne en entrée est une femme (si la probabilité est inférieure à 20\% alors $P(X) = 0$). Enfin, la morale égalitaire (Charline) fera comme la morale de contrôle sauf que pour l'entraînement les informations concernant le genre seront rétirées.
...
\paragraph{Préparation et séparation} Basé sur les résultats de l'analyse précédente, nous allons transformer les variables pour qu'elles puissent être digérées par les modèles. Une fois transformées, nous allons les séparer en deux jeux distincts : un d'entraînement (80\% des données) et un de validation (20\% des données).

\paragraph{Entraînement selon les morales} Pour la morale d'Alice (morale de contrôle) l'entraînement s'effectuera avec toutes les données sorties de la partie précédente en utilisant une recherche d'hyperparamètres aléatoire en validation croisée avec $K=5$\footnote{La validation croisée est une méthode pour que la recherche des hyperparamètres d'un modèle soit plus performante. Par exemple pour $K=5$, nous découpons le jeu d'entraînement en cinq et chaque échantillon servira à entraîner et à valider}. Pour la morale de Bob (morale discriminante), il s'agira du même entraînement, mais avec une brique de sortie retirant 20\% de probabilité pour les femmes. Enfin, pour la morale de Charline (morale égalitaire), avant l'entraînement, nous réaliserons un test de corrélation de Pearson\footnote{Il s'agit d'une mesure de corrélation entre deux varibles quantitatives} et pour les variables ayant un coefficient absolu supérieur à 50\% avec les genre, nous retirerons ces variables pour l'entraînement. Puis nous réalisons un entraînement comme pour Alice. 
...
\paragraph{Contrôle des biais} En exploitant le module d'équité de l'outil TransparentAI, faire un contrôle des biais en utilisant les quatre mesures suivantes : Statistical Parity Difference, Equal Opportunity Difference, Average Odds Difference et Disparate Impact.
...
\begin{itemize}
  ...
  \item L'IA de Charline devrait être la moins biaisée et celle de Bob la plus biaisée, puisque Charline ne prend pas en compte les informations pouvant retraçer jusqu'au genre d'un inidividu, alors que Bob lui, altère négativement les résultats des femmes.
  ...
  \item Si des biais sont observés sur l'IA discriminante, l'atténuation des biais aura un impact sur la performance du modèle.
\end{itemize}
